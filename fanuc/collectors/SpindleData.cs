...
namespace l99.driver.fanuc.collectors
{
    public class SpindleData : FanucMultiStrategyCollector
    {
        ...
        {
            // TODO: does this only return 1st spindle?
        ...
        public override async Task CollectForEachSpindleAsync(short current_spindle, string spindle_name, dynamic spindle_split, dynamic spindle_marker)
        {
            ...
            await strategy.SetNative("diag_lnk", await strategy.Platform.DiagnossByteAsync(400, current_spindle));
            
            // 403 byte             temp of winding spindle motor (C) 0-255
            await strategy.SetNative("diag_temp", await strategy.Platform.DiagnossByteAsync(403, current_spindle));
            ...
            await strategy.SetNative("diag_comms", await strategy.Platform.DiagnossByteAsync(408, current_spindle));
            
            // 410 word             spindle load (%)
            await strategy.SetNative("diag_load_perc", await strategy.Platform.DiagnossWordAsync(410, current_spindle));
            
            // 411 word             spindle load (min)
            await strategy.SetNative("diag_load_min", await strategy.Platform.DiagnossWordAsync(411, current_spindle));
            
            // 417 dword            spindle position coder feedback (detection units)
            await strategy.SetNative("diag_coder", await strategy.Platform.DiagnossDoubleWordAsync(417, current_spindle));
            
            // 418 dword            position loop deviation (detection units)
            await strategy.SetNative("diag_loop_dev", await strategy.Platform.DiagnossDoubleWordAsync(418, current_spindle));
            
            // 425 dword            sync error (detection unit)
            await strategy.SetNative("diag_sync_error", await strategy.Platform.DiagnossDoubleWordAsync(425, current_spindle));
            
            // 445 word             position data (pulse) 0-4095 , valid only when param3117=1
            await strategy.SetNative("diag_pos_data", await strategy.Platform.DiagnossDoubleWordAsync(445, current_spindle));
            
            // 710 word             spindle error
            await strategy.SetNative("diag_error", await strategy.Platform.DiagnossWordAsync(710, current_spindle));
            
            // 712 word             spindle warning
            await strategy.SetNative("diag_warn", await strategy.Platform.DiagnossWordAsync(712,current_spindle));
            
            // 1520 dword           spindle rev count 1 (1000 min)
            await strategy.SetNative("diag_rev_1", await strategy.Platform.DiagnossDoubleWordAsync(1520, current_spindle));
            
            // 1521 dword           spindle rev count 2 (1000 min)
            await strategy.SetNative("diag_rev_2", await strategy.Platform.DiagnossDoubleWordAsync(1521, current_spindle));
            
            // 4902 dword           spindle power consumption (watt)
            await strategy.SetNative("diag_power", await strategy.Platform.DiagnossDoubleWordAsync(4902, current_spindle));
