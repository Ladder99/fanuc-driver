...
namespace l99.driver.fanuc.collectors
{
    ...
    /// This collector extends the ProductionData collector by interrogating files on the C drive when a specific program number becomes active.
    /// Note that subprograms loaded from C drive by NC must have a unique program number (ie. O4321) that can be mapped to a file on the C drive.
    /// </summary>
    // ReSharper disable once UnusedType.Global
    public class ProductionDataExternalSubprogramDetails : FanucMultiStrategyCollector
    {
        public ProductionDataExternalSubprogramDetails(FanucMultiStrategy strategy, object configuration) : base(strategy, configuration)
        {
            if (!Configuration.ContainsKey("lines"))
            {
                Configuration.Add("lines", 60);
            }
        ...
        {
            await Strategy.Apply(typeof(fanuc.veneers.ProductionDataExternalSubprogramDetails), "production", isCompound: true);
        }
