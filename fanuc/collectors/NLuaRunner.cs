    ...
    public class NLuaRunner : FanucCollector2
    {
        ...
        private bool _load_lua_state(string script_text)
        {
            try
            {
                ...
                _lua_table_collector = _lua_state["collector"] as LuaTable;
        ...
        public override async Task CollectForEachAxisAsync(short current_axis, dynamic axis_split, dynamic axis_marker)
        {
            try
            {
                var r = _lua_fnc_collect_axis?.Call(null, _lua_table_collector, this, get("current_path"), current_axis);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public override async Task CollectForEachSpindleAsync(short current_spindle, dynamic spindle_split, dynamic spindle_marker)
        {
            try
            {
                var r = _lua_fnc_collect_spindle?.Call(null, _lua_table_collector, this, get("current_path"), current_spindle);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
