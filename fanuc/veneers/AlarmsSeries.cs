...
public class AlarmsSeries : Veneer
{
    ...
    protected override async Task<dynamic> AnyAsync(dynamic[] nativeInputs, dynamic[] additionalInputs)
    {
        if (nativeInputs[0].success == true)
        {
            ...
            // TODO: added vs removed alarms, who keeps state?
    ...
    private List<dynamic> GetAlarmListFromResponse(AlarmResponse response, short path, string[] axis,
        string[] obsFocasSupport)
    {
        ...
        if (response.Count > 0)
        {
            ...
            for (var x = 0; x <= response.Count - 1; x++)
            {
